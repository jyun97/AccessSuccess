...
class VisionQ14 extends React.Component {
  constructor(props) {
    super(props);
  ...
  handleSubmit() {
    if (this.state.selectedChoice.trim() === '') {
      ...
      this.props.navigation.navigate('VisionQ14');
    }
  }
        ...
        <SafeAreaView style={styles.container}>
          <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
            <View style={styles.inner}>
              ...
              <SelectButton 
                source={require('./images/Q14_Choice1.png')}
                choiceID={'1'}
                activeState={this.state.selectedChoice}
                changeAnswer={(newAnswer) => this.onAnswerChange(newAnswer)}
              />
              <SelectButton 
                  source={require('./images/Q14_Choice2.png')}
                  choiceID={'2'}
                  activeState={this.state.selectedChoice}
                  changeAnswer={(newAnswer) => this.onAnswerChange(newAnswer)}
              />
              <SelectButton 
                  source={require('./images/Q14_Choice3.png')}
                  choiceID={'3'}
                  activeState={this.state.selectedChoice}
                  changeAnswer={(newAnswer) => this.onAnswerChange(newAnswer)}
              />
              <SelectButton 
                  source={require('./images/Q14_Choice4.png')}
                  choiceID={'4'}
                  activeState={this.state.selectedChoice}
                  changeAnswer={(newAnswer) => this.onAnswerChange(newAnswer)}
              />
              ...
              <TouchableOpacity
                style={styles.back}
                onPress={() => this.props.navigation.navigate('VisionQ13')}
              >
